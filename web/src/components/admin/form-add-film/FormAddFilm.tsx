...
export default function FormAddFilm() {
  ...
  async function saveFormData() {
    ...
    try {
      ...
      await uploadVideo(videoInfo.trailer?.id as string, VideoType.TRAILER);
    ...
    async function uploadPoster(id: string) {
      ...
      if (res.status !== 201) {
        throw new Error(
          decodeURI(
            res.headers.get("Message") ?? "Ошибка при загрузке постера.",
          ).replaceAll("+", " "),
        );
      }
    }
    ...
    async function saveMetadata(): Promise<ContentMetadata> {
      ...
      const metadata: ContentMetadataDTO = {
        ...
        subGenresNames: parseSubGenres(form.get("subGenres")?.toString()!),
      ...
      if (res.status !== 201) {
        throw new Error(
          decodeURI(
            res.headers.get("Message") ?? "Ошибка сохранения метаданных.",
          ).replaceAll("+", " "),
        );
      }
