...
const ContentPage = ({}: ContentPageProps): ReactElement => {
    ...
    const filmUrl =
        Constants.STREAMING_SERVER_URL +
        `/stream/standalone/${contentPageMetadata.standaloneVideoShow?.id}`
    ...
    return (
        <View className="relative h-screen w-screen flex-1 bg-cyan-800">
            <View className="fixed left-3 top-3 z-10 h-1/6 w-1/6">
                <BackSign />
            </View>
            ...
            >
                ...
                <Text
                    ...
                    className="mb-2 text-center text-lg text-white"
                >
                    Режиссёр: {contentPageMetadata.filmingGroup.director.name}{' '}
                ...
                >
                    ...
                    <Text className="text-lg text-white">Смотреть</Text>
                </Pressable>
            </BlurView>
        </View>
    )
