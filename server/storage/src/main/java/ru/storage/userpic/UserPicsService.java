...
public class UserPicsService {
     ...
     */
    public List<Pair<String, byte[]>> getInPairs(PicCategory picCategory, String ids) {
     ...
     * @throws S3Exception when an error occurs during the retrieval process
     */
    public List<Pair<String, byte[]>> getInPairs(String s3Folder, String ids) {
    ...
    public void delete(PicCategory picCategory, String ids) {
        List<String> parsedIds = Arrays.asList(ids.split(",")).stream()
            .map(String::trim)
            .toList();
